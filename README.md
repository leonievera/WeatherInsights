...
- If the API call for one city fails, the program should not fail entirly. The data for the other cities should still be processed. I would change my code, so that one API for each city is being made at a time in a loop surrounded with a try/except. If one fails, I can print the error.
